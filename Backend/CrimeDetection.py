...
project = rf.workspace("accidentdetection-en4ti").project("crime_detection-3zobm")
...
# Directory to save crime clips
output_dir = "crime_clips"
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Directory to save crime reports
report_output_dir = "crime_reports"
if not os.path.exists(report_output_dir):
    os.makedirs(report_output_dir)

# Function to generate a PDF report
def generate_pdf_report(crime_details):
    pdf_filename = f'crime_report_{int(time.time())}.pdf'
    ...
    c.drawString(100, height - 50, "Crime Report")
...
def generate_frames():
    ...
    while True:
        ...
        temp_frame_path = 'temp_Crime.jpg'
        ...
        crime_detected = False  # Flag to check if a crime is detected in the current frame
        ...
        for box in predictions:
            ...
            if label in crime_statistics: 
                color = (0, 0, 255)  # Red color for specific labels
                crime_detected = True  # Crime detected in this frame

                # Update crime statistics
                if label in crime_statistics:
                    crime_statistics[label] += 1

                # Send alert
                send_alert(label)

                # Generate PDF report with crime details
                ...
                generate_pdf_report(crime_details)
        ...
        # If a crime is detected, start collecting frames and saving to video
        if crime_detected:
            if not recording:
                # Start recording a new video in the crime_clips folder
                recording = True
                video_filename = f'crime_{int(time.time())}.mp4'
                video_path = os.path.join(output_dir, video_filename)
                
                fourcc = cv2.VideoWriter_fourcc(*'XVID')
                video_writer = cv2.VideoWriter(video_path, fourcc, slow_fps, input_size)
                print(f"Started recording crime video: {video_path}")

            # Append frame to the video
            video_writer.write(frame_resized)

        # If no crime is detected and recording was ongoing, stop recording
        elif recording:
...
def get_crime_statistics():
    return jsonify(crime_statistics)
